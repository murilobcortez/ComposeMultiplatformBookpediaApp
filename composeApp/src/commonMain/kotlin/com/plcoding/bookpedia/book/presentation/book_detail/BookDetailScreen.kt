...
import cmp_bookpedia.composeapp.generated.resources.book_detail_screen_description_unavailable
import cmp_bookpedia.composeapp.generated.resources.book_detail_screen_languages
import cmp_bookpedia.composeapp.generated.resources.book_detail_screen_pages
import cmp_bookpedia.composeapp.generated.resources.book_detail_screen_rating
            ...
            Column(
                ...
                Row(
                    ...
                    state.book.averageRating?.let { rating ->
                        TitledContent(
                          title = stringResource(Res.string.book_detail_screen_rating),
                        ){
                            BookChip {
                                Text(
                                    text = "${round(rating.times(10) / 10.0)}"
                                )
                                Icon(
                    ...
                    state.book.numPages?.let { pageCount ->
                        TitledContent(
                            title = stringResource(Res.string.book_detail_screen_pages),
                        ){
                            BookChip {
                                Text(
                ...
                if(state.book.languages.isNotEmpty()){
                    TitledContent(
                        title = stringResource(Res.string.book_detail_screen_languages),
                        modifier = Modifier
                            .padding(vertical = 8.dp)
                    ){
                        FlowRow(
                ...
                Text(
                    text = stringResource(Res.string.book_detail_screen_synopsis),
                ...
                if(state.isLoading){
                    ...
                    Text(
                        text = if(state.book.description.isNullOrBlank()){
                            stringResource(Res.string.book_detail_screen_description_unavailable)
                        } else {
